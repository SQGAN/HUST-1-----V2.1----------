     ...
     //   static double lon_start = 114.42665029;
      ...
      //  static double lon_start = 0;
        ...
        struct ship_state  //船舶状态信息
        {
            ...
            public double Line_Y;//所跟随直线y
            public double circle_R;//跟随目标半径
        ...
        private ship_state Handle_response_data(ship_state boat)
        {
            ...
            if (flag_ctrl == true)
            {
                using (FileStream fs = new FileStream(@"D:\" + name + ".txt", FileMode.Append))
                {
                    ...
                    string str_data = response_data[3].ToString() + "," + boat.Lat.ToString("0.00000000") + "," + boat.Lon.ToString("0.00000000") + "," + boat.pos_X.ToString("0.000") + "," + boat.pos_Y.ToString("0.000") + "," + boat.phi.ToString("0.0") + ","
                              + boat.GPS_Phi.ToString("0.0") + "," + boat.speed.ToString("0.00") + "," + boat.gear.ToString() + "," + boat.Time.ToString();//将数据转换为字符串
        ...
        private void Control_PF()
        {
            while (flag_ctrl)
            {
                ...
                boat1.Line_Y = double.Parse(line_Y1.Text) * 1000;//获取目标轨迹
                boat1.circle_R = double.Parse(circle_R1.Text)*1000;
                command[3] = Control_fun(boat1);
                command[4] = SpeedControl(boat1);
                SendCommand(command);
                command[3] = 0x47;
                SendCommand(command);
                #endregion

                #region 2号船的控制 领航者，无需控制速度
                command[0] = 0xa2;
                ...
                boat2.Line_Y = double.Parse(line_Y2.Text) * 1000;//获取目标轨迹
                boat2.circle_R = double.Parse(circle_R2.Text) * 1000;

                command[3] = Control_fun(boat2);
                command[4] = 0;
                SendCommand(command);
                command[3] = 0x47;
                SendCommand(command);
                ...
                boat3.Line_Y = double.Parse(line_Y3.Text) * 1000;//获取目标轨迹
                boat3.circle_R = double.Parse(circle_R3.Text) * 1000;

                command[3] = Control_fun(boat3);
                command[4] = SpeedControl(boat3);
                SendCommand(command);
                command[3] = 0x47;
                SendCommand(command);
                #endregion
                Thread.Sleep(200);//控制周期
            }
        }
        double Err_phi_In = 0.0;
        private byte Control_fun(ship_state boat)
        {
            int R = 0;//待返回控制量
            #region 跟踪目标点
            ...
            if (path_mode.Text == "直线")
            {
                ...
                R = (int)(boat.Kp * Err_phi + boat.Ki * Err_phi_In);
            ...
            else if (path_mode.Text == "圆轨迹")
            {
                ...
                double Radius = boat.circle_R;//目标圆半径
        ...
        private byte SpeedControl(ship_state boat)//速度控制
        {
            int U = (int)(boat.K1 * 100 + boat.K2 * (boat2.pos_X - boat.pos_X - boat.dl_err));//设置领航者船2的速度档位10,为了与舵角区分，将命令加上100传输，范围
